# Modified from https://github.com/lian/bitcoin-ruby/blob/f26fa573e8bcb990600314699eb3deecc281a83e/lib/bitcoin/script.rb
# encoding: ascii-8bit
...
class Pushdata
  ...
  def initialize(value, bitcoin_pushdata, bitcoin_pushdata_length)
    @value = value
    @bitcoin_pushdata = bitcoin_pushdata
    @bitcoin_pushdata_length = bitcoin_pushdata_length
  end
end
...
class Bitcoin::Script
  ...
  OPCODES = Hash[*constants.select { |c| c.to_s.start_with?("OP_") }.map{|i| [const_get(i), i.to_s] }.flatten]
  OPCODES[0] = "0"
  OPCODES[81] = "1"
  ...
  def parse(bytes, offset=0)
    ...
    until program.empty?
      ...
      if (opcode > 0) && (opcode < OP_PUSHDATA1)
        ...
        data = program[0, len]
        program = program[len, program.length]
        chunks << data.pack("C*")

        # 0x16 = 22 due to OP_2_16 from_string parsing
        if len == 1 && tmp && tmp <= 22
          chunks[-1] = Pushdata.new(chunks.last, OP_PUSHDATA0, len)
        else
        ...
        unless len > OP_PUSHDATA1 && len <= 0xff
          chunks[-1] = Pushdata.new(chunks.last, OP_PUSHDATA1, len)
        else
        ...
        unless len > 0xff && len <= 0xffff
          chunks[-1] = Pushdata.new(chunks.last, OP_PUSHDATA2, len)
        else
        ...
        unless len > 0xffff # && len <= 0xffffffff
          chunks[-1] = Pushdata.new(chunks.last, OP_PUSHDATA4, len)
        else
    ...
    chunks << Pushdata.new(c, OP_PUSHDATA_INVALID, c.bytesize)
  end
  ...
  def to_string(chunks=nil)
    ...
    (chunks || @chunks).each.with_index{|i,idx|
      ...
      when String
          i.unpack("H*")[0]
      when Pushdata
        "#{i.bitcoin_pushdata}:#{i.bitcoin_pushdata_length}:".force_encoding('binary') + i.value.unpack("H*")[0]
      end
    }
    string
  end
  ...
  def self.pack_pushdata(data)
    ...
    if data.bitcoin_pushdata
      ...
      pack_pushdata_align(data.bitcoin_pushdata, size, data.value)
    else
...
#          end
      ...
      when Pushdata
        if @do_exec
          @debug << "PUSH DATA #{chunk.value.unpack("H*")[0]}"
          @stack << chunk.value
        else
          @debug.pop
        end
  ...
  def pop_int(count=nil)
    ...
    vals = @stack[@stack.length - count, @stack.length]
    @stack = @stack[0, @stack.length - count]
    vals.map{|i| cast_to_bignum(i) }
  end
...
#      invalid if OpenSSL::BN.new(buf.to_s).to_s(0).unpack("N")[0] > 4
    ...
    when String
      ...
      buf.bytes.reverse.reduce(0) { |result, b| b + (result << 8 ) }
#      OpenSSL::BN.new([buf.bytesize].pack("N") + buf.reverse, 0).to_i
  ...
  OPCODES_METHOD = Hash[*instance_methods.select { |m| m.to_s.start_with?("op_") }.map{|m|
